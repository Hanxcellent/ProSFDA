...
def data_load(cfg):
    ...
    input:
        cfg: config file
    output:
        dataset_loaders: a dictionary of dataloaders for training and testing
    """

    datasets = {}
    dataset_loaders = {}
    if cfg.SETTING.MODE == "train":
        train_batch_size = cfg.TEST.BATCH_SIZE

        source_dataset_paths = open(cfg.source_dataset_path).readlines()
        target_dataset_paths = open(cfg.target_dataset_path).readlines()
        test_dataset_paths = open(cfg.test_dataset_path).readlines()
        datasets["source"] = ImageList_idx(source_dataset_paths,
                                             transform=train_image_transform(
                                                     resize_size=cfg.DATA.RESIZE_SIZE,
                                                     crop_size=cfg.DATA.CROP_SIZE))
        dataset_loaders["source"] = DataLoader(datasets["source"], batch_size=train_batch_size,
                                               shuffle=True, num_workers=cfg.NUM_WORKERS,
                                               drop_last=False)

        datasets["target"] = ImageList_idx(target_dataset_paths,
                                           transform=train_image_transform(
                                                   resize_size=cfg.DATA.RESIZE_SIZE,
                                                   crop_size=cfg.DATA.CROP_SIZE))
        dataset_loaders["target"] = DataLoader(datasets["target"], batch_size=train_batch_size,
                                               shuffle=True, num_workers=cfg.NUM_WORKERS,
                                               drop_last=False)

        datasets["test"] = ImageList_idx(test_dataset_paths,
                                         transform=test_image_transform(
                                                 resize_size=cfg.DATA.RESIZE_SIZE,
                                                 crop_size=cfg.DATA.CROP_SIZE))
        dataset_loaders["test"] = DataLoader(datasets["test"], batch_size=train_batch_size * 3,
        ...
        datasets["test"] = ImageList_idx(test_dataset_paths,
                                         transform=test_image_transform(
                                                 resize_size=cfg.DATA.RESIZE_SIZE,
                                                 crop_size=cfg.DATA.CROP_SIZE))
        dataset_loaders["test"] = DataLoader(datasets["test"], batch_size=test_batch_size,
                                             shuffle=False, num_workers=cfg.NUM_WORKERS,
                                             drop_last=False)
...
def tokenize_text(cfg):
    """
    tokenize the text inputs
    input:
        cfg: config file
    output:
        tokenized_prompts: the tokenized text inputs with the format: "a photo of a $category$"
    """
    list_rd = []
    ...
    prompt_prefix = cfg.DIFO.CTX_INIT.replace("_", " ")
