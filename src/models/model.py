...
def get_model(cfg, num_classes):
    if cfg.SETTING.DATASET == "domainnet126":
        cfg.output_path = osp.join(cfg.output_path, 'best_' + cfg.domain[cfg.SETTING.S] + '_2020.pth')
        base_model = ResNetDomainNet126(arch=cfg.MODEL.ARCH, checkpoint_path=cfg.output_path, num_classes=num_classes)
    else:
