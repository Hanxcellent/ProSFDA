...
def generate_class_info(dataset_name):
    ...
    if dataset_name == 'mvtec':
        ...
        obj_list = ['leaper']
    elif dataset_name == 'leaper':
        obj_list = ['leaper']
    elif dataset_name == 'mypaper':
        obj_list = ['mypaper']
...
class Dataset(data.Dataset):
    def __init__(self, root, transform, target_transform, dataset_name, mode = 'test'):
        ...
        self.data_all = []
        meta_info_all = json.load(open(f'{self.root}/meta.json', 'r'))        
        name = self.root.split('/')[-1]
        # print("mode:",mode)
        meta_info = meta_info_all[mode]

        self.cls_names = list(meta_info.keys())
        self.data_by_class_and_anomaly = {cls_name: {'normal': [], 'anomaly': []} for cls_name in self.cls_names}
        for cls_name in self.cls_names:
            self.data_all.extend(meta_info[cls_name])
            for data in meta_info[cls_name]:
                if data['anomaly'] == 0:
                    self.data_by_class_and_anomaly[cls_name]['normal'].append(data)
                else:
                    self.data_by_class_and_anomaly[cls_name]['anomaly'].append(data)
