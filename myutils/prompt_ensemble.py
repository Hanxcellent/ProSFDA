...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        self.n_ctx = design_details["Prompt_length"] # 可学习参数长度
        n_ctx_pos = self.n_ctx
        n_ctx_neg = self.n_ctx
        self.text_encoder_n_ctx = design_details["learnabel_text_embedding_length"] # 嵌入的层数
        ctx_init_pos = ""
        ctx_init_neg = ""
        ...
        normal_num = len(self.state_normal_list) # 正常文本向量的个数
        anormaly_num = len(self.state_anomaly_list)
        self.normal_num = normal_num
        self.anormaly_num = anormaly_num

        if ctx_init_pos and ctx_init_neg: # 这里是用给定的文本描述生成嵌入，将特定字符以外的描述转为可学习参数。默认是不给定
        ...
        self.compound_prompt_projections = _get_clones(single_layer, self.compound_prompts_depth - 1) # 将single_layer克隆self.compound_prompts_depth - 1次
